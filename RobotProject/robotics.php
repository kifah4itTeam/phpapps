...
$sqlqu = "SELECT c.Category_name,p.post_id,p.post_title,p.post_text,p.post_img,MAX(p.post_date) FROM post AS p, category AS c WHERE p.Category_id = c.Category_id Group by p.Category_id";
$data = mysqli_query($connection,$sqlqu);
$id2="SELECT Category_Id,Category_name FROM category";
$GetCat = mysqli_query($connection,$id2);
$row;
?>
<!doctype html>
<html>
...
<a href="<?php echo("article.php?id=".$row['post_id'])?>"><img src="<?php echo($row['post_img']) ?>" width="300" height="300"  alt=""/></a>
 <div class="caption">
<p class="post-title"><?php echo $row['Category_name'] ?></p>

</div>
</div>
</div>
<?php /*?><div class="col-xs-6 col-sm-6 col-md-3">
   <div class="thumbnail">
<a href="<?php echo("article.php?id=".$row['post_id'])?>"><img src="<?php echo($row['post_img'])?>" width="300" height="300"  alt=""/></a>
 <div class="caption">
